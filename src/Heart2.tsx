...
//Foto's of video's importeren => uit map in array steken
const images = require.context('/imgs', true);
const imageList = images.keys().map(image => images(image));
const videos = require.context('/videos', true);
const videosList = videos.keys().map(video => videos(video));

const Heart2 = () => {
    const [password, setPassword] = useState<any>('hey');
    const [imgOrVideo, setImgOrVideo] = useState(true);
    ...
    return (
        password === "hey" ?
            ...
            <div>
                ...
                {imgOrVideo ?
                    <button onClick={() => { setImgOrVideo(item => !item); handleRangeChange2(0) }} >Verander naar video's</button>
                    : <button onClick={() => setImgOrVideo(item => !item)} >Verander naar foto's</button>
                }
                ...
                <div className='head'  >
                    {imgOrVideo ?
                        <>
                            ...
                            <div className='leftLabel' >Totaal aantal video's:</div>
                            <div className='rightLabel' >{videosList.length}</div> <br /><br />
                        ...
                        <div>
                            <div className='leftLabel' >Totaal aantal video's:</div>
                            <div className='rightLabel' >{videosList.length}</div> <br /><br />
                            <div className='leftLabel' >De grootte van het hart is:</div>
                            <div className='rightLabel' >{sum(array1voor, array1achter, 1) + sum(array2voor, array2achter, 2) + sum(array3voor, array3achter, 3)}</div> <br />
                        </div>
                    }
                </div>
                ...
                <div>
                    <div className='twoTopPartsOfHeartShape'>
                        <div> {Array(array1voor).fill(null).map((item, index) => <OneRowOfImagesDiv stars={index + array1achter} teksten={teksten} images={imageList} videos={videosList} imgOrVideo={imgOrVideo} />)}</div>
                        <div> {Array(array1voor).fill(null).map((item, index) => <OneRowOfImagesDiv stars={index + array1achter} teksten={teksten} images={imageList} videos={videosList} imgOrVideo={imgOrVideo} />)}</div>
                    </div>

                    <div className='bottomPartOfHeartShape'>
                        {Array(array2voor).fill(null).map((item, index) => <OneRowOfImagesDiv stars={array2achter - index} teksten={teksten} images={imageList} videos={videosList} imgOrVideo={imgOrVideo} />)}
                        {Array(array3voor).fill(null).map((item, index) => <OneRowOfImagesDiv stars={array3achter - index} teksten={teksten} images={imageList} videos={videosList} imgOrVideo={imgOrVideo} />)}
                    </div>
                </div>
